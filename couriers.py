...
class Couriers:
    ...
    def __init__(self, splash=None):
        ...
        need_driver_handler = [
            ...
            if isinstance(courier.handler, SeleniumHandler)
        ]
...
class DPD(Courier):
    ...
    def parse_content(self, tree):
        ...
        for evt in timeline:
            ...
            events.append(
                dict(
                    date=datetime.strptime(f"{date} {hour}", "%d/%m/%Y %H:%M").replace(
                        tzinfo=get_localzone()
                    ),
                    status=location,
                    label=label,
...
class Chronopost(LaPoste):
    ...
    def get_url_for_browser(self, idship):
        json = LaPoste.get_content(
            LaPoste, idship
        )  # super() calls with a ChronoPost class instance
        if json:
